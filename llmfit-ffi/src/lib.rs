...
use llmfit_core::analysis::{build_model_fits, InstalledIndex};
...
pub extern "C" fn llmfit_create_context() -> *mut LlmFitContext {
    match panic::catch_unwind(|| {
        ...
        Box::into_raw(Box::new(LlmFitContext { specs, db, installed }))
    }) {
        Ok(ptr) => ptr,
        Err(_) => {
...
pub extern "C" fn llmfit_get_last_error() -> *mut c_char {
    ...
    if err_msg.is_empty() {
        ...
        CString::new(err_msg).map(|c| c.into_raw()).unwrap_or(std::ptr::null_mut())
    }
}
...
#[no_mangle]
pub extern "C" fn llmfit_get_model_info(ctx_ptr: *mut LlmFitContext, model_name: *const c_char) -> *mut c_char {
    if ctx_ptr.is_null() {
        set_error("Context pointer is null");
        return std::ptr::null_mut();
    }
    ...
    let models: Vec<_> = ctx.db.get_all_models()
        .into_iter()
