...
fn estimate_tps(
    ...
    // For heterogeneous multi-GPU, use minimum bandwidth — slowest card is the bottleneck.
    let bandwidth = if system.gpus.len() > 1 {
        let bws: Vec<f64> = system.gpus.iter().filter_map(|g| gpu_memory_bandwidth_gbps(&g.name)).collect();
        if bws.is_empty() { gpu_memory_bandwidth_gbps(gpu_name) } else { bws.into_iter().reduce(f64::min) }
    } else {
